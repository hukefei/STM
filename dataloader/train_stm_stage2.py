...
class YOUTUBE2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        frame_first = random.randint(0,7)
        frame_second = random.randint(8, 20)
        frame_third = random.randint(21, 35)
        if frame_third > self.num_frames[video]-1:
            frame_first = random.randint(0,5)
            frame_second = random.randint(6,13)
            frame_third = random.randint(14,20)
            if frame_third > self.num_frames[video]-1:
                frame_first = random.randint(0, 5)
                frame_second = random.randint(6, 10)
                frame_third = random.randint(11, 15)
                if frame_third > self.num_frames[video]-1:
                    frame_first = random.randint(0, self.num_frames[video]-3)
                    frame_second = frame_first + 1
                    frame_third = frame_second + 1
