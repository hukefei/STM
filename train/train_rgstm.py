...
from models.model_rgstm import STM
...
def parse_args():
    ...
    parser.add_argument('--work_dir', type=str, default='./exp/rgstm_res50_tf_e-6', help='the dir to save models.pth and logs and masks')
    parser.add_argument("--mode", type=str, default='train', help="train or val")
    parser.add_argument('--load_from', type=str, default='/home/yuanlei/stm/exp/rgstm_res50/ckpt/youtube/ckpt_0113.pth')
    # train
    ...
    parser.add_argument("--gpu", type=str, default='3', help="0; 0,1; 0,3; etc")
...
def Run_video(args, Fs, Ms, num_frames, name, Mem_every=None, Mem_number=None):
    ...
    for t in tqdm.tqdm(range(1, num_frames)):
        ...
        # segment
        array_low = (0.5 * torch.ones(preEs.shape)).cuda()
        mask_q = torch.max(array_low, preEs.clone())
        mask_q = mask_q
        logits = model([Fs[:, :, t], this_keys_m, this_values_m, mask_q])  # B 2 h w
...
if __name__ == '__main__':
    ...
    if args.year==2016:
        ...
        val_loader_lyuan = data.DataLoader(val_dataset_lyuan, batch_size=1, shuffle=False, num_workers=2, pin_memory=True)
    else:
        val_dataset_2016 = DAVIS(DAVIS_ROOT, phase='val', imset='2016/val.txt', resolution='480p',
                                 separate_instance=False, only_single=False, target_size=(864, 480))
        val_loader_2016 = data.DataLoader(val_dataset_2016, batch_size=1, shuffle=False, num_workers=2, pin_memory=True)
    ...
    if args.mode == "val":
        ...
        if args.train_data == 'youtube':
            ...
            train_dataset = DAVIS3(DAVIS_ROOT, phase='train', imset= '/2017/val.txt', resolution='480p',
