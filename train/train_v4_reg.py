...
from models.pycls.tools.model_stm_s import STM
...
def parse_args():
    ...
    parser.add_argument('--work_dir', type=str, default='./exp/stm_s_reg800_v4.3', help='the dir to save models.pth and logs and masks')
    parser.add_argument("--mode", type=str, default='train', help="train or val")
    parser.add_argument('--load_from', type=str, default='/home/yuanlei/stm/exp/stm_s_reg800_v4.1/ckpt/youtube/ckpt_0075.pth')
    # train
    ...
    parser.add_argument("--year", type=int, default=0, help="year")
    # val
...
def Run_video(args, Fs, Ms, num_frames, name, Mem_every=None, Mem_number=None):
    ...
    key, value, _ = model([Fs[:, :, 0], Ms[:, :, 0].float()])
    [b,c,h,w] = key.shape
    ...
    for t in tqdm.tqdm(range(1, num_frames)):
        ...
        pre_key[:,:,0], pre_value[:,:,0], r1= model([preFs, preEs])
        # flops, params = profile(model, inputs=([preFs, preEs]))
        # print('******memory:', flops, params)
        # print('preFs, preEs:', preFs.shape, preEs.shape)
        # np.save(args.work_dir + '/dogsJump_frame1_encoderM_reg.npy', r1.detach().cpu().numpy())
        # sys.exit()
        ...
        # segment
        # print('this_key_m, this_values_m:', this_keys_m.shape, this_values_m.shape)
        logits, p_m2, p_m3 = model([Fs[:, :, t], this_keys_m, this_values_m]) # B 2 h w
        # flops, params = profile(model, inputs=([Fs[:, :, t], this_keys_m, this_values_m]))
        # print('******segment:', flops, params)
        em = F.softmax(logits, dim=1)[:, 1] # B h w

        # em_numpy = em.detach().cpu().numpy()
        # np.save('em.npy', em_numpy)
        # sys.exit()

        Es[:,0,t] = em

        #  calculate loss on cuda
        Ms_cuda = Ms[:,0,t].clone().cuda()
...
def validate(args, val_loader, model):
    ...
    for seq, batch in enumerate(val_loader):
        ...
        with torch.no_grad():
            name = info['name']
            loss_video, video_mIou = Run_video(args, Fs, Ms, num_frames, name, Mem_every=1000, Mem_number=None)
...
if __name__ == '__main__':
    ...
    # checkpoints = torch.load('/cfs/yuanlei/my_stm/models/pycls/weights/RegNetY-800MF_dds_8gpu.pyth')['model_state']
    # checkpoints_stm = dict()
    # for key in checkpoints.keys():
    #     checkpoints_stm['module.Encoder_M.'+key] = checkpoints[key]
    #     checkpoints_stm['module.Encoder_Q.'+key] = checkpoints[key]
    # model.load_state_dict(checkpoints_stm, strict=False)
    # del checkpoints
    # del checkpoints_stm
    ...
    if args.load_from:
        ...
        ckpt = torch.load(args.load_from)['state_dict']
        # ckpt_encoder = dict()
        # for key in list(ckpt.keys()):
        #     if 'D' not in key:
        #         ckpt_encoder[key] = ckpt[key]
